...
# =========================
# DEEP NAIJA SNIPER ENGINE
# =========================
class NaijaSniperEngine:
    def __init__(self):
    ...
    async def scrape_source(self, source, title: str):
        """Scrapes a specific source for direct download buttons."""
        print(f"--- Sniper: Targeting {source['name']} for '{title}' ---")
        async with httpx.AsyncClient(headers={"User-Agent": self.ua}, follow_redirects=True, timeout=12.0) as client:
            try:
                ...
                # 2. Find first relevant result page
                links = []
                # Look for links containing the title
                for a in soup.find_all('a', href=True):
                    if title.lower()[:4] in a.text.lower() and source['base'] in a['href']:
                        ...
                        try:
                            ...
                            for btn in page_soup.select('a.download, a[href*="download"], a.btn-primary, a.btn-success'):
                                href = btn['href']
                                if href.startswith('http') and ('.mp4' in href.lower() or '.mkv' in href.lower() or 'download' in href.lower()):
                                    links.append({
                                        "quality": f"{source['name']}: {btn.text.strip()[:20] or 'Direct Link'}",
                                        "resolution": "HD", "format": "MP4", "size": "Fast",
                                        "url": href
                                    })
                        except: continue
                        if links: break # Found links on this site, move to next site
                return links
            except: return []
...
async def get_movie_details(subject_id: str, media_type: str = Query("movie", alias="type"), title: Optional[str] = None):
    try:
        # NAIJA SNIPER: Scrape the whitelisted sites in parallel
        links = await sniper.get_all_mirrors(title or "")
        
        if links:
            return {"success": True, "data": {"id": subject_id, "title": title or "Media", "qualities": links, "mediaType": media_type, "platform": "Naija Sniper Engine"}}
        
        raise Exception(f"No mirrors found on Naija servers for '{title}'.")
