...
try:
    ...
    if os.path.exists(service_account_path):
        ...
        print(f"--- Firebase Admin Initialized via File ---")
    else:
...
# Download folder - Use /tmp for serverless compatibility
...
async def stream_video(url: str, request: Request, referer: Optional[str] = None):
    # Direct redirect for high performance streaming
...
async def broadcast_notification(request: Request):
    ...
    try:
        ...
        for doc in users_docs:
            ...
            if u_data.get('fcmToken'): tokens.append(u_data.get('fcmToken'))
        ...
        if tokens:
            for j in range(0, len(tokens), 500):
                message_obj = messaging.MulticastMessage(
                    ...
                    data={"url": "/notifications", "unreadCount": "1"},
                    tokens=tokens[j:j + 500],
                )
...
async def start_movie_download(request: Request, background_tasks: BackgroundTasks):
    ...
    async def run_download():
        try:
            ...
            # FALLBACK TO MOVIEBOX DOWNLODER (If no direct URL)
            ...
            async def progress_hook(tracker: DownloadTracker):
                if not download_events[task_id].is_set(): await download_events[task_id].wait()
            ...
            async def perform_download(query_text):
                try:
                    if media_type == "series" and season and episode:
                        results = await downloader.download_tv_series(query_text, season=int(season), episode=int(episode), limit=1, yes=True, quality=target_q, dir=DOWNLOAD_DIR, progress_hook=progress_hook)
                        if results and isinstance(results, dict):
                            for s_val in results.values():
                                for e_val in s_val.values():
                                    if isinstance(e_val, dict) and 'video' in e_val: return e_val['video']
                        return None
                    else:
                        res = await downloader.download_movie(query_text, yes=True, quality=target_q, dir=DOWNLOAD_DIR, progress_hook=progress_hook)
                        if res and isinstance(res, tuple) and len(res) > 0: return res[0]
                        return None
...
async def get_movie_details(subject_id: str, media_type: str = Query("movie", alias="type"), title: Optional[str] = None, season: Optional[int] = None, episode: Optional[int] = None):
    try:
        ...
        # 3. CRITICAL BYPASS: Synthetic model fallback
        if not target_item:
